    ...
    : trackerClient(qq), q(qq)
{
    ...
    errorString = QT_TRANSLATE_NOOP(TorrentClient, "_Unknown error_");
...
void TorrentClientPrivate::setError(TorrentClient::Error errorCode)
{
    ...
    case TorrentClient::TorrentParseError:
        errorString = QT_TRANSLATE_NOOP(TorrentClient, "Invalid magshare data");
        break;
...
{
    QLOG_DEBUG() << "TorrentClient::setTorrent fileName:" << fileName;
    QFile file(fileName);
...
bool TorrentClient::setTorrent(const QByteArray &torrentData)
{
    ...
    /* using our simple magparse */
    if (!d->metaInfo.magparse(torrentData)) {
        d->setError(TorrentParseError);
        return false;
    }
...
{
    QLOG_DEBUG() << "TorrentClient::setDumpedState";
...
{
    QLOG_DEBUG() << "TorrentClient::dumpedState";
...
int TorrentClient::seedCount() const
{
    ...
    QLOG_DEBUG() << "TorrentClient::seedCount" << tmp;
    return tmp;
}
...
void TorrentClient::start()
{
    if (d->state != Idle)
        return;
    QLOG_DEBUG() << "TorrentClient::start";
...
void TorrentClient::stop()
{
    ...
    // mag - stop uploading check
    if (d->uploadScheduleTimer) {
        killTimer(d->uploadScheduleTimer);
    }
}
...
void TorrentClient::setPaused(bool paused)
{
    ...
    QLOG_DEBUG() << "TorrentClient::setPaused";
...
{
    QLOG_DEBUG() << "TorrentClient::sendToPeer";
...
void TorrentClient::fullVerificationDone()
{
    ...
    if (!server->isListening()) {
        ...
        for (int i = ServerMinPort; i <= ServerMaxPort; ++i) {
            ...
            {
                QLOG_DEBUG() << "TorrentClient::fullVerificationDone - peerwire server port:" << QString::number(i);
                break;
            }
        }
        if (!server->isListening()) {
...
{
    QLOG_DEBUG() << "TorrentClient::pieceVerified";
...
{
    QLOG_DEBUG() << "TorrentClient::handleFileError";
...
{
    QLOG_DEBUG() << "TorrentClient::connectToPeers";
...
{
    QLOG_DEBUG() << "TorrentClient::weighedFreePeers";
...
{
    QLOG_DEBUG() << "TorrentClient::setupIncomingConnection";
    // Connect signals
    initializeConnection(client);
...
{
    QLOG_DEBUG() << "TorrentClient::setupOutgoingConnection";
...
{
    QLOG_DEBUG() << "TorrentClient::initializeConnection";
...
{
    QLOG_DEBUG() << "TorrentClient::removeClient";
...
{
    QLOG_DEBUG() << "TorrentClient::peerPiecesAvailable";
...
{
    QLOG_DEBUG() << "TorrentClient::peerRequestsBlock";
...
{
    QLOG_DEBUG() << "TorrentClient::blockReceived";
...
{
    QLOG_DEBUG() << "TorrentClient::peerWireBytesWritten";
...
{
    QLOG_DEBUG() << "TorrentClient::peerWireBytesReceived";
    if (!d->transferRateTimer)
        d->transferRateTimer = startTimer(RateControlTimerDelay);
...
{
    QLOG_DEBUG() << "TorrentClient::blocksLeftForPiece";
...
{
    QLOG_DEBUG() << "TorrentClient::scheduleUploads";
...
{
    QLOG_DEBUG() << "TorrentClient::scheduleDownloads";
...
{
    QLOG_DEBUG() << "TorrentClient::schedulePieceForClient";
...
{
    QLOG_DEBUG() << "TorrentClient::requestMore";
...
{
    QLOG_DEBUG() << "TorrentClient::requestBlocks";
...
{
    QLOG_DEBUG() << "TorrentClient::peerChoked";
...
{
    QLOG_DEBUG() << "TorrentClient::peerUnchoked";
...
{
    QLOG_DEBUG() << "TorrentClient::addToPeerList";
...
{
    QLOG_DEBUG() << "TorrentClient::trackerStopped";
    d->setState(Idle);
    emit stopped();
}
...
{
    QLOG_DEBUG() << "TorrentClient::updateProgress";
